...
*/
type Simple [1]classdb.WorkerThreadPool
func (self Simple) AddTask(action gd.Callable, high_priority bool, description string) int {
	gc := gd.GarbageCollector(); _ = gc
	return int(int(Expert(self).AddTask(action, high_priority, gc.String(description))))
}
func (self Simple) IsTaskCompleted(task_id int) bool {
	gc := gd.GarbageCollector(); _ = gc
	return bool(Expert(self).IsTaskCompleted(gd.Int(task_id)))
}
func (self Simple) WaitForTaskCompletion(task_id int) gd.Error {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Error(Expert(self).WaitForTaskCompletion(gd.Int(task_id)))
}
func (self Simple) AddGroupTask(action gd.Callable, elements int, tasks_needed int, high_priority bool, description string) int {
	gc := gd.GarbageCollector(); _ = gc
	return int(int(Expert(self).AddGroupTask(action, gd.Int(elements), gd.Int(tasks_needed), high_priority, gc.String(description))))
}
func (self Simple) IsGroupTaskCompleted(group_id int) bool {
	gc := gd.GarbageCollector(); _ = gc
	return bool(Expert(self).IsGroupTaskCompleted(gd.Int(group_id)))
}
func (self Simple) GetGroupProcessedElementCount(group_id int) int {
	gc := gd.GarbageCollector(); _ = gc
	return int(int(Expert(self).GetGroupProcessedElementCount(gd.Int(group_id))))
}
func (self Simple) WaitForGroupTaskCompletion(group_id int) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).WaitForGroupTaskCompletion(gd.Int(group_id))
}
...
//go:nosplit
func (self class) AsWorkerThreadPool() Expert { return self[0].AsWorkerThreadPool() }
...
func init() {classdb.Register("WorkerThreadPool", func(ptr gd.Pointer) any {var class Expert; class[0].SetPointer(ptr); return class })}
