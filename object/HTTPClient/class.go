...
*/
type Simple [1]classdb.HTTPClient
func (self Simple) ConnectToHost(host string, port int, tls_options [1]classdb.TLSOptions) gd.Error {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Error(Expert(self).ConnectToHost(gc.String(host), gd.Int(port), tls_options))
}
func (self Simple) SetConnection(connection [1]classdb.StreamPeer) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).SetConnection(connection)
}
func (self Simple) GetConnection() [1]classdb.StreamPeer {
	gc := gd.GarbageCollector(); _ = gc
	return [1]classdb.StreamPeer(Expert(self).GetConnection(gc))
}
func (self Simple) RequestRaw(method classdb.HTTPClientMethod, url string, headers gd.PackedStringArray, body []byte) gd.Error {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Error(Expert(self).RequestRaw(method, gc.String(url), headers, gc.PackedByteSlice(body)))
}
func (self Simple) Request(method classdb.HTTPClientMethod, url string, headers gd.PackedStringArray, body string) gd.Error {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Error(Expert(self).Request(method, gc.String(url), headers, gc.String(body)))
}
func (self Simple) Close() {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).Close()
}
func (self Simple) HasResponse() bool {
	gc := gd.GarbageCollector(); _ = gc
	return bool(Expert(self).HasResponse())
}
func (self Simple) IsResponseChunked() bool {
	gc := gd.GarbageCollector(); _ = gc
	return bool(Expert(self).IsResponseChunked())
}
func (self Simple) GetResponseCode() int {
	gc := gd.GarbageCollector(); _ = gc
	return int(int(Expert(self).GetResponseCode()))
}
func (self Simple) GetResponseHeaders() gd.PackedStringArray {
	gc := gd.GarbageCollector(); _ = gc
	return gd.PackedStringArray(Expert(self).GetResponseHeaders(gc))
}
func (self Simple) GetResponseHeadersAsDictionary() gd.Dictionary {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Dictionary(Expert(self).GetResponseHeadersAsDictionary(gc))
}
func (self Simple) GetResponseBodyLength() int {
	gc := gd.GarbageCollector(); _ = gc
	return int(int(Expert(self).GetResponseBodyLength()))
}
func (self Simple) ReadResponseBodyChunk() []byte {
	gc := gd.GarbageCollector(); _ = gc
	return []byte(Expert(self).ReadResponseBodyChunk(gc).Bytes())
}
func (self Simple) SetReadChunkSize(bytes int) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).SetReadChunkSize(gd.Int(bytes))
}
func (self Simple) GetReadChunkSize() int {
	gc := gd.GarbageCollector(); _ = gc
	return int(int(Expert(self).GetReadChunkSize()))
}
func (self Simple) SetBlockingMode(enabled bool) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).SetBlockingMode(enabled)
}
func (self Simple) IsBlockingModeEnabled() bool {
	gc := gd.GarbageCollector(); _ = gc
	return bool(Expert(self).IsBlockingModeEnabled())
}
func (self Simple) GetStatus() classdb.HTTPClientStatus {
	gc := gd.GarbageCollector(); _ = gc
	return classdb.HTTPClientStatus(Expert(self).GetStatus())
}
...
func (self Simple) QueryStringFromDict(fields gd.Dictionary) string {
	gc := gd.GarbageCollector(); _ = gc
	return string(Expert(self).QueryStringFromDict(gc, fields).String())
}
...
*/
//go:nosplit
func (self class) ConnectToHost(host gd.String, port gd.Int, tls_options object.TLSOptions) int64 {
...
//go:nosplit
func (self class) SetConnection(connection object.StreamPeer)  {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
...
//go:nosplit
func (self class) GetConnection(ctx gd.Lifetime) object.StreamPeer {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
...
//go:nosplit
func (self class) AsHTTPClient() Expert { return self[0].AsHTTPClient() }


//go:nosplit
func (self Simple) AsHTTPClient() Simple { return self[0].AsHTTPClient() }
...
func init() {classdb.Register("HTTPClient", func(ptr gd.Pointer) any {var class Expert; class[0].SetPointer(ptr); return class })}
