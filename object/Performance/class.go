...
*/
type Simple [1]classdb.Performance
func (self Simple) GetMonitor(monitor classdb.PerformanceMonitor) float64 {
	gc := gd.GarbageCollector(); _ = gc
	return float64(float64(Expert(self).GetMonitor(monitor)))
}
func (self Simple) AddCustomMonitor(id string, callable gd.Callable, arguments gd.Array) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).AddCustomMonitor(gc.StringName(id), callable, arguments)
}
func (self Simple) RemoveCustomMonitor(id string) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).RemoveCustomMonitor(gc.StringName(id))
}
func (self Simple) HasCustomMonitor(id string) bool {
	gc := gd.GarbageCollector(); _ = gc
	return bool(Expert(self).HasCustomMonitor(gc.StringName(id)))
}
func (self Simple) GetCustomMonitor(id string) gd.Variant {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Variant(Expert(self).GetCustomMonitor(gc, gc.StringName(id)))
}
func (self Simple) GetMonitorModificationTime() int {
	gc := gd.GarbageCollector(); _ = gc
	return int(int(Expert(self).GetMonitorModificationTime()))
}
func (self Simple) GetCustomMonitorNames() gd.ArrayOf[gd.StringName] {
	gc := gd.GarbageCollector(); _ = gc
	return gd.ArrayOf[gd.StringName](Expert(self).GetCustomMonitorNames(gc))
}
...
//go:nosplit
func (self class) AsPerformance() Expert { return self[0].AsPerformance() }


//go:nosplit
func (self Simple) AsPerformance() Simple { return self[0].AsPerformance() }
...
func init() {classdb.Register("Performance", func(ptr gd.Pointer) any {var class Expert; class[0].SetPointer(ptr); return class })}
