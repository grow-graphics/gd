...
*/
type Simple [1]classdb.RootMotionView
func (self Simple) SetAnimationPath(path gd.NodePath) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).SetAnimationPath(path)
}
func (self Simple) GetAnimationPath() gd.NodePath {
	gc := gd.GarbageCollector(); _ = gc
	return gd.NodePath(Expert(self).GetAnimationPath(gc))
}
...
func (self Simple) SetCellSize(size float64) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).SetCellSize(gd.Float(size))
}
func (self Simple) GetCellSize() float64 {
	gc := gd.GarbageCollector(); _ = gc
	return float64(float64(Expert(self).GetCellSize()))
}
func (self Simple) SetRadius(size float64) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).SetRadius(gd.Float(size))
}
func (self Simple) GetRadius() float64 {
	gc := gd.GarbageCollector(); _ = gc
	return float64(float64(Expert(self).GetRadius()))
}
func (self Simple) SetZeroY(enable bool) {
	gc := gd.GarbageCollector(); _ = gc
	Expert(self).SetZeroY(enable)
}
func (self Simple) GetZeroY() bool {
	gc := gd.GarbageCollector(); _ = gc
	return bool(Expert(self).GetZeroY())
}
...
//go:nosplit
func (self class) AsRootMotionView() Expert { return self[0].AsRootMotionView() }


//go:nosplit
func (self Simple) AsRootMotionView() Simple { return self[0].AsRootMotionView() }
...
func init() {classdb.Register("RootMotionView", func(ptr gd.Pointer) any {var class Expert; class[0].SetPointer(ptr); return class })}
