...
func GetInterface(idx int) gdclass.TextServer {
	once.Do(singleton)
	return gdclass.TextServer(class(self).GetInterface(gd.Int(idx)))
}
...
func FindInterface(name string) gdclass.TextServer {
	once.Do(singleton)
	return gdclass.TextServer(class(self).FindInterface(gd.NewString(name)))
}
...
*/
//go:nosplit
func (self class) GetInterface(idx gd.Int) gdclass.TextServer {
	var frame = callframe.New()
...
*/
//go:nosplit
func (self class) GetPrimaryInterface() gdclass.TextServer {
	var frame = callframe.New()
	var r_ret = callframe.Ret[[1]uintptr](frame)
