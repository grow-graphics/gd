...
func New() Go {
	...
	object := gc.API.ClassDB.ConstructObject(gc, gc.StringName("Curve"))
...
func (self Go) OnRangeChanged(cb func()) {
	gc := gd.GarbageCollector(); _ = gc
	self[0].AsObject().Connect(gc.StringName("range_changed"), gc.Callable(cb), 0)
}
...
func init() {classdb.Register("Curve", func(ptr gd.Pointer) any {var class class; class[0].SetPointer(ptr); return class })}
