...
func (self Go) GetCellTileData(coords gd.Vector2i) gdclass.TileData {
	gc := gd.GarbageCollector(); _ = gc
	return gdclass.TileData(class(self).GetCellTileData(gc, coords))
}
...
*/
func (self Go) GetUsedCellsById() gd.ArrayOf[gd.Vector2i] {
	gc := gd.GarbageCollector(); _ = gc
	return gd.ArrayOf[gd.Vector2i](class(self).GetUsedCellsById(gc, gd.Int(-1), gd.Vector2i{-1, -1}, gd.Int(-1)))
}
...
*/
func (self Go) GetPattern(coords_array gd.ArrayOf[gd.Vector2i]) gdclass.TileMapPattern {
	gc := gd.GarbageCollector(); _ = gc
	return gdclass.TileMapPattern(class(self).GetPattern(gc, coords_array))
}
...
*/
func (self Go) SetCellsTerrainConnect(cells gd.ArrayOf[gd.Vector2i], terrain_set int, terrain int) {
	gc := gd.GarbageCollector(); _ = gc
...
[b]Note:[/b] To work correctly, this method requires the [TileMapLayer]'s TileSet to have terrains set up with all required terrain combinations. Otherwise, it may produce unexpected results.
*/
func (self Go) SetCellsTerrainPath(path gd.ArrayOf[gd.Vector2i], terrain_set int, terrain int) {
	gc := gd.GarbageCollector(); _ = gc
...
func New() Go {
	...
	object := gc.API.ClassDB.ConstructObject(gc, gc.StringName("TileMapLayer"))
	return *(*Go)(unsafe.Pointer(&object))
}

func (self Go) TileMapData() []byte {
	gc := gd.GarbageCollector(); _ = gc
		return []byte(class(self).GetTileMapDataAsArray(gc).Bytes())
}

func (self Go) SetTileMapData(value []byte) {
	gc := gd.GarbageCollector(); _ = gc
	class(self).SetTileMapDataFromArray(gc.PackedByteSlice(value))
}
...
*/
//go:nosplit
func (self class) GetCellTileData(ctx gd.Lifetime, coords gd.Vector2i) gdclass.TileData {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
	callframe.Arg(frame, coords)
...
*/
//go:nosplit
func (self class) GetPattern(ctx gd.Lifetime, coords_array gd.ArrayOf[gd.Vector2i]) gdclass.TileMapPattern {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
...
func init() {classdb.Register("TileMapLayer", func(ptr gd.Pointer) any {var class class; class[0].SetPointer(ptr); return class })}
