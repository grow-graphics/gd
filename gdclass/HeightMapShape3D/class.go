...
func New() Go {
	...
	object := gc.API.ClassDB.ConstructObject(gc, gc.StringName("HeightMapShape3D"))
...
func (self Go) MapData() []float32 {
	gc := gd.GarbageCollector(); _ = gc
		return []float32(class(self).GetMapData(gc).AsSlice())
}

func (self Go) SetMapData(value []float32) {
	gc := gd.GarbageCollector(); _ = gc
	class(self).SetMapData(gc.PackedFloat32Slice(value))
}
...
func init() {classdb.Register("HeightMapShape3D", func(ptr gd.Pointer) any {var class class; class[0].SetPointer(ptr); return class })}
