...
Custom code to generate previews. Please check [code]file_dialog/thumbnail_size[/code] in [EditorSettings] to find out the right size to do previews at.
	...
	type EditorResourcePreviewGenerator interface {
		...
		Generate(resource gdclass.Resource, size gd.Vector2i, metadata gd.Dictionary) gdclass.Texture2D
		...
		//Care must be taken because this function is always called from a thread (not the main thread).
		//[param metadata] dictionary can be modified to store file-specific metadata that can be used in [method EditorResourceTooltipPlugin._make_tooltip_for_path] (like image size, sample length etc.).
		GenerateFromPath(path string, size gd.Vector2i, metadata gd.Dictionary) gdclass.Texture2D
...
func New() Go {
	...
	object := gc.API.ClassDB.ConstructObject(gc, gc.StringName("EditorResourcePreviewGenerator"))
...
func init() {classdb.Register("EditorResourcePreviewGenerator", func(ptr gd.Pointer) any {var class class; class[0].SetPointer(ptr); return class })}
