...
*/
type Go [1]classdb.ENetConnection
...
*/
func (self Go) CreateHostBound(bind_address string, bind_port int) gd.Error {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Error(class(self).CreateHostBound(gc.String(bind_address), gd.Int(bind_port), gd.Int(32), gd.Int(0), gd.Int(0), gd.Int(0)))
}
...
[b]Note:[/b] It is necessary to create a host in both client and server in order to establish a connection.
*/
func (self Go) CreateHost() gd.Error {
	gc := gd.GarbageCollector(); _ = gc
...
*/
func (self Go) ConnectToHost(address string, port int) gdclass.ENetPacketPeer {
	gc := gd.GarbageCollector(); _ = gc
	return gdclass.ENetPacketPeer(class(self).ConnectToHost(gc, gc.String(address), gd.Int(port), gd.Int(0), gd.Int(0)))
}
...
*/
func (self Go) Service() gd.Array {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Array(class(self).Service(gc, gd.Int(0)))
}
...
*/
func (self Go) ChannelLimit(limit int) {
	gc := gd.GarbageCollector(); _ = gc
...
*/
func (self Go) Broadcast(channel int, packet []byte, flags int) {
	gc := gd.GarbageCollector(); _ = gc
	class(self).Broadcast(gd.Int(channel), gc.PackedByteSlice(packet), gd.Int(flags))
}
...
*/
func (self Go) Compress(mode classdb.ENetConnectionCompressionMode) {
	gc := gd.GarbageCollector(); _ = gc
...
*/
func (self Go) DtlsServerSetup(server_options gdclass.TLSOptions) gd.Error {
	gc := gd.GarbageCollector(); _ = gc
...
*/
func (self Go) DtlsClientSetup(hostname string) gd.Error {
	gc := gd.GarbageCollector(); _ = gc
	return gd.Error(class(self).DtlsClientSetup(gc.String(hostname), ([1]gdclass.TLSOptions{}[0])))
}
...
*/
func (self Go) RefuseNewConnections(refuse bool) {
	gc := gd.GarbageCollector(); _ = gc
...
*/
func (self Go) PopStatistic(statistic classdb.ENetConnectionHostStatistic) float64 {
	gc := gd.GarbageCollector(); _ = gc
...
*/
func (self Go) GetMaxChannels() int {
	gc := gd.GarbageCollector(); _ = gc
...
*/
func (self Go) GetPeers() gd.ArrayOf[gdclass.ENetPacketPeer] {
	gc := gd.GarbageCollector(); _ = gc
	return gd.ArrayOf[gdclass.ENetPacketPeer](class(self).GetPeers(gc))
}
...
*/
func (self Go) SocketSend(destination_address string, destination_port int, packet []byte) {
	gc := gd.GarbageCollector(); _ = gc
	class(self).SocketSend(gc.String(destination_address), gd.Int(destination_port), gc.PackedByteSlice(packet))
}
...
func New() Go {
	...
	object := gc.API.ClassDB.ConstructObject(gc, gc.StringName("ENetConnection"))
...
*/
//go:nosplit
func (self class) ConnectToHost(ctx gd.Lifetime, address gd.String, port gd.Int, channels gd.Int, data gd.Int) gdclass.ENetPacketPeer {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
	...
	var ret gdclass.ENetPacketPeer
...
*/
//go:nosplit
func (self class) DtlsServerSetup(server_options gdclass.TLSOptions) int64 {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
...
*/
//go:nosplit
func (self class) DtlsClientSetup(hostname gd.String, client_options gdclass.TLSOptions) int64 {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
...
*/
//go:nosplit
func (self class) GetPeers(ctx gd.Lifetime) gd.ArrayOf[gdclass.ENetPacketPeer] {
	var selfPtr = self[0].AsPointer()
	var frame = callframe.New()
	var r_ret = callframe.Ret[uintptr](frame)
	...
	var ret = mmm.New[gd.Array](ctx.Lifetime, ctx.API, r_ret.Get())
	frame.Free()
	return gd.TypedArray[gdclass.ENetPacketPeer](ret)
}
/*
Sends a [param packet] toward a destination from the address and port currently bound by this ENetConnection instance.
This is useful as it serves to establish entries in NAT routing tables on all devices between this bound instance and the public facing internet, allowing a prospective client's connection packets to be routed backward through the NAT device(s) between the public internet and this host.
...
func (self class) AsENetConnection() GD { return *((*GD)(unsafe.Pointer(&self))) }
func (self Go) AsENetConnection() Go { return *((*Go)(unsafe.Pointer(&self))) }
...
func init() {classdb.Register("ENetConnection", func(ptr gd.Pointer) any {var class class; class[0].SetPointer(ptr); return class })}
