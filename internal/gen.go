...
func genEnum(pkg string, decl, code io.Writer, prefix string, enum Enum) {
	...
	if decl != nil {
		...
		fmt.Fprintln(code)
		fmt.Fprintf(code, "type %v = internal.%[1]v\n", name)

		if name == "ExtensionInitializationLevel" || name == "VariantType" {
...
func generate() error {
	...
	fmt.Fprintln(out, `package internal`)
	...
	fmt.Fprintln(all, `package internal`)
	fmt.Fprintln(all)
	...
	fmt.Fprintln(enums)
	fmt.Fprintln(enums, `import "grow.graphics/gd/internal"`)
	fmt.Fprintln(enums)
	...
	fmt.Fprintln(classdb)
	fmt.Fprintln(classdb, `import "grow.graphics/gd/internal"`)
	fmt.Fprintln(classdb)

	for _, enum := range spec.GlobalEnums {
		genEnum("gd", all, enums, "", enum)
	}
	...
	for _, class := range spec.BuiltinClasses {
		for _, enum := range class.Enums {
			genEnum("gd", all, enums, class.Name, enum)
		}
	...
	for _, class := range spec.BuiltinClasses {
		...
		for _, method := range class.Methods {
			methodCall(all, class, method, callBuiltin)
		}
	}
	fmt.Fprintf(out, "\t}\n")
	fmt.Fprintf(out, "}\n")
	...
	for _, class := range spec.Classes {
		...
		if class.Name != "Object" {
			fmt.Fprintf(classdb, "type %v = internal.%v \n", class.Name, class.Name)
		}
		if class.Inherits != "" {
			fmt.Fprintf(all, "type %[1]v struct {Class[%[1]v, %v]}\n", class.Name, class.Inherits)
		} else {
			fmt.Fprintf(all, "type %[1]v struct {Class[%[1]v, Pointer]}\n", class.Name)
		}
		if class.Inherits != "" {
			var i = 1
			super := classDB[class.Inherits]
			for super.Name != "" {
				fmt.Fprintf(all, "\nfunc (self %[1]v) %[2]v() %[2]v { return *self%s }\n", class.Name, super.Name, strings.Repeat(".Super()", i))
				i++
		...
		for _, method := range class.Methods {
			methodCall(all, class, method, callDefault)
		}
...
func methodCall(w io.Writer, class Class, method Method, ctype callType) {
	switch class.Name {
	...
	if method.IsVirtual {
		fmt.Fprintf(w, "\nfunc (%s) %s(impl func(ptr unsafe.Pointer, ctx Context", class.Name, method.Name)
		for _, arg := range method.Arguments {
			fmt.Fprint(w, ", ")
