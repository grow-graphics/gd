...
func generate() error {
	...
	cnv, err := os.Create("./internal/classdb/new.go")
	if err != nil {
		return err
	}
	fmt.Fprintln(cnv, `//go:build !generate`)
	fmt.Fprintln(cnv)
	fmt.Fprintln(cnv, `// Code generated by the generate package DO NOT EDIT`)
	fmt.Fprintln(cnv, `package gd`)
	fmt.Fprintln(cnv)
	fmt.Fprintln(cnv, `import gd "grow.graphics/gd/internal"`)
	fmt.Fprintln(cnv)
	fmt.Fprintln(cnv, `func init() {`)
	fmt.Fprintln(cnv, `	gd.ObjectAs = func(name string, ptr gd.Pointer) any {`)
	fmt.Fprintln(cnv, `		switch name {`)
	defer func() {
		fmt.Fprintln(cnv, `		}`)
		fmt.Fprintln(cnv, "var obj gd.Object")
		fmt.Fprintln(cnv, "obj.SetPointer(ptr)")
		fmt.Fprintln(cnv, `return obj`)
		fmt.Fprintln(cnv, `	}`)
		fmt.Fprintln(cnv, `}`)
	}()
	...
	for _, class := range spec.Classes {
		...
		if class.Name != "Object" {
			if singletons[class.Name] {
				...
				fmt.Fprintf(cnv, "\t\tvar obj %v\n", name)
				fmt.Fprintf(cnv, "\t\tobj.SetPointer(ptr)\n")
				fmt.Fprintf(cnv, "\t\treturn obj\n")
			}
