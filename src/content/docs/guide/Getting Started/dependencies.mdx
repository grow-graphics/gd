---
title: Installing Dependencies
slug: guide/dependencies
sidebar:
  order: 2
---
import { Aside } from '@astrojs/starlight/components';
import { Tabs, TabItem } from '@astrojs/starlight/components';

## Go

If you haven't already done so, you will need to [Download and Install Go](https://go.dev/dl/) so you will be able to compile any `.go` source files into native executables.

After Go is installed, make sure `$GOPATH/bin` is part of your `PATH`, so that any tools that you decide to `go install` will be available for use in your terminal.

<Tabs>
<TabItem label="Windows">
```powershell
setx PATH "%PATH%;%GOPATH%\bin"
```
</TabItem>
<TabItem label="macOS">
```sh
echo 'export PATH="$PATH:$GOPATH/bin"' >> ~/.zshrc && source ~/.zshrc
```
</TabItem>
<TabItem label="Linux">
```sh
echo 'export PATH="$PATH:$GOPATH/bin"' >> ~/.bashrc && source ~/.bashrc
```
</TabItem>
</Tabs>

## Godot
The `gd` command will look for `godot` in your `$PATH` - make sure you have the right version installed (typically this will be the latest stable version of Godot).

https://godotengine.org/download/

<Aside>On Linux and MacOS, the `gd` command line tool will attempt to automatically download and use the correct version of `godot` for you if it's not already installed.</Aside>

## Zig
In order for Go code to work together with the C++ code in the Godot Engine, you'll need a C compiler. We officially
support [The Zig Compiler](https://ziglang.org/download/) as it supports full cross-compilation, if you just want to
build for your current platform, you may prefer to use your system C compiler, make sure `CC` is set accordingly.

<Aside> `graphics.gd` can build projects for any supported platforms on any operating system (this includes building for macOS on Windows/Linux without needing access to Apple hardware).</Aside>

<Tabs>
<TabItem label="Windows">
Please ensure that Zig is added to your `PATH` after you've extracted it to a convieniant location on your computer.

```powershell
setx PATH "%PATH%;C:\place\you\extracted\zig-windows-x86_64-your-version"
```
</TabItem>
<TabItem label="macOS">
You can use [Homebrew](https://brew.sh/) to install Zig:
```sh
brew install zig
```
</TabItem>
<TabItem label="Linux">
You should be able to install Zig using your package manager.
```sh
snap install zig
```
</TabItem>
</Tabs>
