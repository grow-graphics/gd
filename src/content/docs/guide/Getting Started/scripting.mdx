...
slug: guide/scripting-reference
---

`graphics.gd` is compatible with the builtin `.gd` scripting language of the engine, please refer to this
...
`.gd` identifiers are `snake_case`, class names are `PascalCase`, scripts typically use an underscore
to denote a private property.
...
Go file can independently import other Go packages by their path (typically a Github path, or a path within the
project's Go module).
