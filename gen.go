...
func generate() error {
	...
	fmt.Fprintf(api, "\tExtension ExtensionAPI\n")
	...
	for _, class := range spec.BuiltinClasses {
		...
		for _, creator := range class.Constructors {
			if class.Name == "Nil" {
				continue
			}
			var from = ""
			if len(creator.Arguments) == 1 {
				if strings.Title(creator.Arguments[0].Type) == class.Name {
					from = "Copy"
				} else {
					from = "From" + strings.Title(creator.Arguments[0].Type)
				}
			}
			if len(creator.Arguments) > 1 {
				from = fmt.Sprint(creator.Index)
				var same string
				for _, arg := range creator.Arguments {
					if same == "" || same == arg.Type {
						same = arg.Type
					} else {
						same = ""
						break
				...
				if same != "" {
					from = "From" + strings.Title(same) + "s"
				}
			}
			if class.Name == "Plane" && creator.Index == 5 {
				from = "FromNormalAndPoint"
			}
			if class.Name == "Color" && creator.Index == 3 {
				from = "FromRGB"
			}
			if class.Name == "Color" && creator.Index == 4 {
				from = "FromRGBA"
			}
			fmt.Fprintf(api, "\t%v_New%v func(", strings.Title(convertType("gd", "", class.Name)), from)
			for i, arg := range creator.Arguments {
				fmt.Fprintf(api, "%v %v", fixReserved(arg.Name), convertType("gd", arg.Meta, arg.Type))
				if i < len(creator.Arguments)-1 {
					fmt.Fprint(api, ", ")
				}
			}
			fmt.Fprintf(api, ") %v\t`index:\"%v\"`\n", convertType("gd", "", class.Name), creator.Index)
		}
	...
	for _, class := range spec.Classes {
		...
		for _, method := range class.Methods {
			...
			fmt.Fprintf(all, "\tfunc (self class%v) %v(", class.Name, convertName(method.Name))
