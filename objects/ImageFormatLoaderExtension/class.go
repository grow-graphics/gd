...
func New() Instance {
	...
	return Instance{*(*classdb.ImageFormatLoaderExtension)(unsafe.Pointer(&object))}
}
...
Loads the content of [param fileaccess] into the provided [param image].
*/
func (class) _load_image(impl func(ptr unsafe.Pointer, image objects.Image, fileaccess objects.FileAccess, flags classdb.ImageFormatLoaderLoaderFlags, scale gd.Float) error) (cb gd.ExtensionClassCallVirtualFunc) {
	return func(class any, p_args gd.UnsafeArgs, p_back gd.UnsafeBack) {
		var image = objects.Image{pointers.New[classdb.Image]([3]uintptr{gd.UnsafeGet[uintptr](p_args, 0)})}
		defer pointers.End(image[0])
		var fileaccess = objects.FileAccess{pointers.New[classdb.FileAccess]([3]uintptr{gd.UnsafeGet[uintptr](p_args, 1)})}
...
func init() {
	classdb.Register("ImageFormatLoaderExtension", func(ptr gd.Object) any {
		return [1]classdb.ImageFormatLoaderExtension{*(*classdb.ImageFormatLoaderExtension)(unsafe.Pointer(&ptr))}
	})
}
