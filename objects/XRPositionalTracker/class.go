...
*/
func (self Instance) GetPose(name string) objects.XRPose {
	return objects.XRPose(class(self).GetPose(gd.NewStringName(name)))
}
...
*/
//go:nosplit
func (self class) GetPose(name gd.StringName) objects.XRPose {
	var frame = callframe.New()
	callframe.Arg(frame, pointers.Get(name))
	var r_ret = callframe.Ret[[1]uintptr](frame)
...
func (self Instance) OnPoseChanged(cb func(pose objects.XRPose)) {
	self[0].AsObject().Connect(gd.NewStringName("pose_changed"), gd.NewCallable(cb), 0)
}

func (self Instance) OnPoseLostTracking(cb func(pose objects.XRPose)) {
