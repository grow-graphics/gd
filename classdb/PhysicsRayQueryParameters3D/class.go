...
func Create(from Vector3.XYZ, to Vector3.XYZ) [1]gdclass.PhysicsRayQueryParameters3D {
	self := Instance{}
	return [1]gdclass.PhysicsRayQueryParameters3D(class(self).Create(gd.Vector3(from), gd.Vector3(to), gd.Int(4294967295), [1]gd.Array{}[0]))
}
...
func New() Instance {
	...
	return Instance{*(*gdclass.PhysicsRayQueryParameters3D)(unsafe.Pointer(&object))}
}
