...
func (self Instance) CreateDataChannel(label string) [1]gdclass.WebRTCDataChannel { //gd:WebRTCPeerConnection.create_data_channel
	return [1]gdclass.WebRTCDataChannel(class(self).CreateDataChannel(String.New(label), Dictionary.Nil))
}
...
func (self Instance) SetLocalDescription(atype string, sdp string) error { //gd:WebRTCPeerConnection.set_local_description
	return error(gd.ToError(class(self).SetLocalDescription(String.New(atype), String.New(sdp))))
}
...
func (self Instance) SetRemoteDescription(atype string, sdp string) error { //gd:WebRTCPeerConnection.set_remote_description
	return error(gd.ToError(class(self).SetRemoteDescription(String.New(atype), String.New(sdp))))
}
...
func (self Instance) AddIceCandidate(media string, index int, name string) error { //gd:WebRTCPeerConnection.add_ice_candidate
	return error(gd.ToError(class(self).AddIceCandidate(String.New(media), gd.Int(index), String.New(name))))
}
...
*/
//go:nosplit
func (self class) SetLocalDescription(atype String.Readable, sdp String.Readable) gd.Error { //gd:WebRTCPeerConnection.set_local_description
	var frame = callframe.New()
...
*/
//go:nosplit
func (self class) SetRemoteDescription(atype String.Readable, sdp String.Readable) gd.Error { //gd:WebRTCPeerConnection.set_remote_description
	var frame = callframe.New()
	callframe.Arg(frame, pointers.Get(gd.InternalString(atype)))
	callframe.Arg(frame, pointers.Get(gd.InternalString(sdp)))
	var r_ret = callframe.Ret[gd.Error](frame)
...
*/
//go:nosplit
func (self class) AddIceCandidate(media String.Readable, index gd.Int, name String.Readable) gd.Error { //gd:WebRTCPeerConnection.add_ice_candidate
	var frame = callframe.New()
